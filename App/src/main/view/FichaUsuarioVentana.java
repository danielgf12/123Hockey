...
public class FichaUsuarioVentana extends JFrame {
    ...
    public FichaUsuarioVentana(Usuario loggedUser, Usuario targetUser) {
        ...
        // al hacer click en la foto, si estamos en modo edición, permitir cambiarla
        lblFoto.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                int res = chooser.showOpenDialog(FichaUsuarioVentana.this);
                if (res == JFileChooser.APPROVE_OPTION) {
                    File file = chooser.getSelectedFile();
                    try {
                        byte[] bytes = Files.readAllBytes(file.toPath());
                        targetUser.setFotoUsuario(bytes);
                        // Persistimos el cambio en la BBDD
                        new UsuarioDAO().actualizarUsuario(targetUser);
                        // Actualizamos el icono en pantalla
        ...
        dgbc.gridx = 1;
        datosPanel.add(cbCategoria, dgbc);

        // Nacimiento
        dgbc.gridx = 2;
        datosPanel.add(createLabel("Nacimiento:"), dgbc);
        txtNacimiento = createField(
                formatearFecha(info!=null?info.getFechaNacimiento():null)
        );
        dgbc.gridx = 3;
        datosPanel.add(txtNacimiento, dgbc);
        ...
        dgbc.gridx = 3;
        datosPanel.add(cbPosicion, dgbc);

        // Asistencia obligatoria / Partidos jugados
        ...
        dgbc.gridx = 1;
        datosPanel.add(txtAsistOblig, dgbc);
        ...
        dgbc.gridx = 3;
        datosPanel.add(txtPartidos, dgbc);
        ...
        btnDelete.addActionListener(e -> {
            if (JOptionPane.showConfirmDialog(this,
                ...
                new AsistenciaDAO().eliminarPorUsuario(targetUser.getId());
    ...
    private Date parseFecha(String s){
        try{return new SimpleDateFormat("d/MM/yyyy").parse(s);}
    ...
    private ImageIcon loadIcon(String name,int w,int h){
        ...
        Image img=(u!=null?new ImageIcon(u).getImage()
